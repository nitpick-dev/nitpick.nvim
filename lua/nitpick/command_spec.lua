...
describe("command", function()
	describe("completions", function()
		test("return all completions when cmd is emtpy", function()
			...
			assert.are_same({ "comment", "start", "rstart", "end", "activity", "notes", "tasks", "authorize", "next" }, cmp)
		end)
	...
	describe("dispatch", function()
		...
		test("tasks command", function()
			local tasks = stub(nitpick, "open_tasks")

			assert.is_true(command.dispatch({ "tasks" }))
			assert.stub(tasks).was.called(1)
		end)
